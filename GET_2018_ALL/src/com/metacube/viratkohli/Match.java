...
public class Match {
	 ...
	 */
	public void addBowler(int ball, String name) throws MatchException {
	 ...
	 */
	public void totalBall() {
		for (Bowler bowler : bowlers) {
			totalBowl += bowler.getBall();
		}
	}
		...
		while (totalBowl != 0) {
			...
			if (orderSize == 0) {
				...
				bowlerOrder.get(bowlerOrder.size() - 1).setBowl(
						bowlerOrder.get(bowlerOrder.size() - 1).getBall() + 1);
			}
			bowlers.get(bolwerIndex).setBowl(
					bowlers.get(bolwerIndex).getBall() - 1);
			totalBowl--;
		}
		return bowlerOrder;
	}
	...
	private int getMaxBowlBolwer() {
		...
		int maxBowls = bowlers.get(0).getBall();
		int index = 0;
		for (int i = 0; i < numberOfBowlers; i++) {
			int bowls = bowlers.get(i).getBall();
			if (bowls > maxBowls) {
				maxBowls = bowls;
				index = i;
			}
